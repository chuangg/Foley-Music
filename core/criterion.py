...
class SoundNetMSELoss(_Loss):
    __constants__ = ['reduction']

    def __init__(self, reduction='mean'):
        super().__init__(reduction=reduction)
        self.sound_net = SoundNet()
        self.sound_net.load_weights()
        self.sound_net.eval()
        self.mse_loss = nn.MSELoss(reduction=reduction)

    def forward(self, input, target):
        input_feat = self.sound_net(input)
        target_feat = self.sound_net(target)
        return self.mse_loss(input_feat, target_feat)


class ReinforceLoss(_Loss):
    __constants__ = ['reduction']

    def __init__(self, reduction='mean', coeff=1e-3):
        super().__init__(reduction=reduction)
        self.coeff = coeff

    def forward(self, model_output: Tensor, mse_loss: Tensor):
        """

        :param input: [B, T, C] from model output
        :param target: [B, 1] from mse loss
        :return: scalar
        """
        ...
        loss = m.log_prob(action) * mse_loss - m.entropy() * self.coeff
        if self.reduction == 'mean':
