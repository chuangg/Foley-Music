...
class Control:
    ...
    def to_pitch_histogram_array(self):
        phist = np.array(self.pitch_histogram, dtype=np.float32)
        return phist
...
class ControlSeq:
    ...
    def to_array(self):
        ...
        ndens1 = np.zeros([len(ndens), feat_dims['note_density']], dtype=np.float32)
        ndens1[np.arange(len(ndens)), ndens] = 1.  # [steps, dens_dim]

        phist = [control.pitch_histogram for control in self.controls]
        phist = np.array(phist, dtype=np.float32)

        # if phist.ndim != 2:
        #     print('phist', phist.shape)
        #     print('ndense1', ndens1.shape)

        phist = phist.reshape(-1, feat_dims['pitch_histogram'])

        return np.concatenate([
            ndens1,  # [steps, dens_dim] density index
            phist  # [steps, hist_dim] 0-255
        ], 1)  # [steps, dens_dim:hist_dim]
...
if __name__ == '__main__':
    ...
    path = sys.argv[1] if len(sys.argv) > 1 else 'datasets/midi/ecomp/BLINOV02.mid'
